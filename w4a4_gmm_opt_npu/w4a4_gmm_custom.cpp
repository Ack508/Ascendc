...
, GM_ADDR workspace, GM_ADDR tilingGm1)
{
    ...
    */
    ...
    if(core_num >= CORE_NUM) //用CORE_NUM个核并行,这里CORE_NUM的值为4
    {
        core_num=CORE_NUM-1;
    }
    ...
    AscendC::SliceInfo srcSliceInfoInw_scale[2] = {{static_cast<uint32_t>(core_num*N), static_cast<uint32_t>(core_num*N+N-1), 0, 1, static_cast<uint32_t>(CORE_NUM*N)}, {0, E-1, 0, 1, E}};
    ...
    AscendC::SliceInfo srcSliceInfoInw[3] = {{static_cast<uint32_t>(core_num*N), static_cast<uint32_t>(core_num*N+N-1), 0, 1, static_cast<uint32_t>(CORE_NUM*N)}, {0, K-1, 0, 1, K}, {0, E-1, 0, 1, E}};
    ...
    AscendC::DataCopy(wLocal1, wGm, dstSliceInfow, srcSliceInfow, 3);//w切片数据搬运，三维
    AscendC::DataCopy(x_scaleLocal1, x_scaleGm, x_scaleTILE_LENGTH);
    AscendC::DataCopy(w_scaleLocal1, w_scaleGm, dstSliceInfow_scale, srcSliceInfow_scale, 2);//w_scale切片数据搬运
    ...
    AscendC::DataCopy(yGm[core_num*N], yLocal2, intriParams);
